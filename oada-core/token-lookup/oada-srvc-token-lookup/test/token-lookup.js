  ...
  describe('.token-lookup', () => {
    it('should be able to perform a token-lookup', (done) => {
      ...
      requester.send(t).then((msg) => {
        ...
        expect(msg.doc.authorizationid).to.equal(
          oadaLib.examples('authorizations')[0]._id
        );
        expect(msg.doc.user_id).to.equal(frankid);
        expect(msg.doc.scope).to.be.instanceof(Array);
        expect(msg.doc.bookmarks_id).to.equal(
          'resources/default:resources_bookmarks_123'
        );
        expect(msg.doc.shares_id).to.equal(
          'resources/default:resources_shares_123'
        );
        expect(msg.doc.client_id).to.equal(
          'jf93caauf3uzud7f308faesf3@provider.oada-dev.com'
        );
    ...
    it('should error when token does not exist', (done) => {
      ...
      requester.send(t).then((msg) => {
        ...
        expect(msg.doc.shares_id).to.equal(null);

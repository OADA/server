    ...
    socket.on('message', function message (msg) {
      ...
      if (!msg.requestId) {
        const err = {
          status: 400,
          headers: [],
          data: new OADAError('Bad Request', 400, 'Missing `requestId`')
        }
        socket.send(JSON.stringify(err))
        return
      }
      ...
      if (!msg.headers || !msg.headers.authorization) {
        const err = {
          status: 400,
          headers: [],
          data: new OADAError('Bad Request', 400, 'Missing `authorization`')
        }
        err.requestId = msg.requestId
      ...
      if (
        ['unwatch', 'watch', 'head', 'get', 'put', 'post', 'delete'].includes(
      ...
      switch (msg.method.toLowerCase()) {
        ...
        case 'post':
          request.method = 'post'
          request.url = msg.path
          request.data = JSON.stringify(msg.data)
          break
      ...
      axios(request)
        .then(function (res) {
          ...
          function handleChange (change) {
            ...
            if (
              change &&
              jsonpointer.get(change.change.body, path_leftover) !== undefined
            ) {
              const message = {
            ...
            if (request.headers['x-oada-rev']) {
              ...
              oadaLib.resources
                ...
                .then(async function (rev) {
                  ...
                  if (
                    parseInt(rev) - parseInt(request.headers['x-oada-rev']) >=
                    revLimit
                  ) {
                    trace(
                      'REV WAY OUT OF DATE',
                    ...
                    var resource = await oadaLib.resources.getResource(
                      resourceId
                    )
                    socket.send(
                      JSON.stringify({

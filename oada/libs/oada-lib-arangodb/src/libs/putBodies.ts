 ...
 */
export async function savePutBody(body: string) {
  // the _id comes back in the response to save
  return await collection.save(`{"body":${body}}`);
}

export async function getPutBody(id: string): Promise<string | null> {
  const { body } = await collection.document(id);
  return body;
}

export async function removePutBody(id: string) {
